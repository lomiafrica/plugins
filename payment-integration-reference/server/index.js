...
app.post("/api/webhooks/lomi", (req, res) => {
  ...
  if (payload.event === "PAYMENT_SUCCEEDED") {
    console.log("[webhook] PAYMENT_SUCCEEDED", payload.data);
  } else if (payload.event === "PAYMENT_FAILED") {
    console.log("[webhook] PAYMENT_FAILED", payload.data);
  } else {
    console.log("[webhook] event", payload.event || eventName || "unknown");
  }
...
app.listen(port, () => {
  console.log(`Payment reference server running on ${appBaseUrl}`);
  console.log(`Using lomi base URL: ${lomiBaseUrl}`);
  console.log(`Using checkout base URL: ${checkoutBaseUrl}`);
});
